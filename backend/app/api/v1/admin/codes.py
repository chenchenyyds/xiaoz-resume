...
from app.schemas.redeem import GenerateCodeReq, GenerateCodeResp, CodeListResp, CodeListItem
...
async def generate(
    ...
    result = redeem_service.generate_batch(db, req.type, req.count, req.valid_days, req.batch_id)
...
async def list_codes(
    ...
    return CodeListResp(
        items=[CodeListItem(
            id=c.id, code_mask=c.code_mask, type=c.type, points=c.points,
            status=c.status, batch_id=c.batch_id, user_id=c.user_id,
...
async def export(
    ...
    items, _ = admin_service.list_codes(db, batch_id=batch_id, status=status, type_=type, page=1, page_size=10000)

    buf = io.StringIO()
    writer = csv.writer(buf)
    writer.writerow(["code_mask", "type", "points", "status", "user_id", "used_at", "created_at"])
    for c in items:
        writer.writerow([c.code_mask, c.type, c.points, c.status, c.user_id or "", c.used_at or "", c.created_at])
