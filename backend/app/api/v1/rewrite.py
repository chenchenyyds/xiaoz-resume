...
from app.schemas.rewrite import PartialRewriteReq, FullRewriteReq, PartialRewriteResp, FullRewriteResp
...
async def partial_rewrite(
    ...
    logger.info(f"[api.rewrite.partial] user={user.id} enter text_len={len(req.text or '')} title={req.title!r}")
    # 限频
    max_per_hour = get_config_int(db, "limit.partial_per_hour", 10)
    check_rate_limit(user.id, "partial_rewrite", max_per_hour, 3600)
    # 统一包装:与项目其它 API 一致
    return ok(rewrite_service.partial_rewrite(db, user.id, req.text, req.title, req.style_hint))
...
async def full_rewrite(
    ...
    logger.info(f"[api.rewrite.full] user={user.id} enter file_id={req.file_id} has_jd={bool(req.jd_text)} jd_len={len(req.jd_text or '')}")
    # 限频
    max_per_hour = get_config_int(db, "limit.full_per_hour", 3)
    check_rate_limit(user.id, "full_rewrite", max_per_hour, 3600)
    # 统一包装:与项目其它 API 一致
    return ok(rewrite_service.full_rewrite(db, user.id, req.file_id, req.jd_text, req.style_hint))
